  ...
  <div class="page-guide-card-div">
    <div class="page-guide-card-div-left">
      <a-breadcrumb>
        ...
        <a-breadcrumb-item>编辑用例</a-breadcrumb-item>
      </a-breadcrumb>
      <span>您可以在此页面编辑测试用例！</span>
    </div>
    <div class="page-guide-card-div-right">
      <a-button class="margin-right-10" @click="onClickOfRefresh">刷新</a-button>
  ...
  <div class="case-card-div">
    ...
    <div class="case-card-left-div">
      ...
      <a-card style="margin-top: 10px" size="small" title="基础信息">
        ...
        <a-select v-model:value="caseState.productId" :options="proBoxOptions" @change="onChangeOfForProduct" style="margin-top: 10px; width: 100%" placeholder="请选择所属产品(product)"></a-select>
        <a-select v-model:value="caseState.workgroupId" :options="groupBoxOptions" style="margin-top: 10px; width: 100%" placeholder="请选择所属业务组"></a-select>
        <a-select v-model:value="caseState.status" :options="statusOptions" style="margin-top: 10px; width: 100%" placeholder="请选择测试用例状态"></a-select>
        <div style="border: 1px solid #d9d9d9; margin-top: 10px">
          ...
          <div style="width: 100%; padding-left: 20px; padding-top: 10px">
            <a-radio-group v-model:value="caseState.environmentId" name="envRaidoGroup"
        ...
        <div style="border: 1px solid #d9d9d9; margin-top: 10px">
          ...
          <div style="width: 100%; padding-left: 20px; padding-top: 10px">
            <a-radio-group v-model:value="caseState.iterationId" name="iteraRaidoGroup"
    ...
    <div class="case-card-right-div">
      ...
      <a-textarea :disabled=true v-model:value="yamlState"
                  style="width: 100%; height: calc(100% - 90px); margin-top: 10px; resize: none; border: none;"/>
    </div>
  </div>
</template>
...
<script>
  ...
  import {isNumDecimals, isPositiveInteger} from "../../utils/datautils";
  import store from "../../store/store";

  const tag = "/index/cases/edit";
  ...
  export default defineComponent({
    ...
    setup(props, ctx) {
      ...
      // state: 测试用例数据
      const caseState = ref({
        id: 0, number: "", title: "", description: "", productId: [], workgroupId: [],
      ...
      const caseDetails = (params) => {
        logger.redundancy(tag, "exec function caseDetails...");
        ...
        caseDetailsApi(params).then((res) => {
          ...
          if(res.status === 200) {
            let data = res.data;
            if(data.code === 200) {
              caseState.value.id = data.data.ID;
      ...
      const caseEdit = (body) => {
        ...
        caseEditApi(body).then((res) => {
          logger.info(tag, "res:", res);
          if (res.status === 200) {
            let data = res.data;
            if (data.code === 200) {
              openSuccessNotification("编辑测试用例成功！");
            }else {
              openFailedNotification(data.message);
            }
          }else {
          ...
          logger.info(tag, "call caseAddApi complete!")
        })
      }
