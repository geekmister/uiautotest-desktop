<template>
  ...
  <!-- 详情抽屉 -->
  <a-drawer v-model:visible="detailsState.visibleDetails" title="设备详情"
            placement="right"
            @after-visible-change="afterVisibleChangeOfDetails"
            size="large">
    <a-table
        :columns="detailsState.columns"
        :dataSource="detailsState.data"
        :pagination="false"
        style="width: 100%;">
    </a-table>
  </a-drawer>
  ...
  export default defineComponent({
    ...
    setup() {
      ...
      const onClickOfDetails = (udid) => {
        detailsState.value.visibleDetails = true;
        deviceDetails({udid: udid});
      }
      ...
      const afterVisibleChangeOfDetails = (bool) => {
        console.log('visibleDetails', bool);
      };
      ...
      const deviceDetails = (params) => {
        logger.redundancy(tag, "exec function deviceDetails...");
